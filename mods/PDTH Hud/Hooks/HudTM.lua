...
function HUDTeammate:init(i, teammates_panel, is_player, height)
	...
	self.health_h = (is_player and 129 or 32.5) * pdth_hud.loaded_options.Ingame.Hud_scale
	self.health_w = (is_player and 64 or 16) * pdth_hud.loaded_options.Ingame.Hud_scale
	...
	if not main_player then
        ...
        radial_health_panel:set_left(self._player_panel:left() + 5.5)
	end
	...
	local condition_timer = teammate_panel:text({
		...
		color = Color.white,
		blend_mode = "normal",
		font_size = (tweak_data.hud_players.timer_size - 10) * pdth_hud.loaded_options.Ingame.Hud_scale,
...
function HUDTeammate:set_health(data)
	...
	if not self._main_player then
		radial_health:set_h(self.health_h - y_offsetsmall)
...
function HUDTeammate:set_armor(data)
	...
	if not self._main_player then
		radial_shield:set_h(self.health_h - y_offsetsmall)
...
function HUDTeammate:set_state(state)
	...
	--teammate_panel:set_alpha(is_player and 1 or 0)
...
function HUDTeammate:_animate_timer_flash()
	...
	while t < 0.5 do
		...
		condition_timer:set_font_size(math.lerp((tweak_data.hud_players.timer_size - 10) * pdth_hud.loaded_options.Ingame.Hud_scale, (tweak_data.hud_players.timer_flash_size - 10) * pdth_hud.loaded_options.Ingame.Hud_scale, n))
	end
...
function HUDTeammate:set_character_portrait()
	...
	if self._main_player then
        ...
        character_text:set_bottom(radial_health_panel:h() - 2)
	end
	if self._main_player then
