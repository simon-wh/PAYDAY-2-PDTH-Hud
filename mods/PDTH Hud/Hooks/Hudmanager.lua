...
local mugshot_talk = HUDManager.set_mugshot_talk
local mugshot_voice = HUDManager.set_mugshot_voice
...
function HUDManager:set_mugshot_talk(id, active)
		...
		if data.id == id then
			if not data.peer_id then
				if self._teammate_panels then
					if self._teammate_panels[id] then
						if self._teammate_panels[id]._panel then
							if self._teammate_panels[id]._panel:child("talk") then
								self._teammate_panels[id]._panel:child("talk"):set_visible(active)
							end
						end
					end
				end
	...
	return mugshot_talk(self, id, active)
end
...
function HUDManager:set_mugshot_voice(id, active)
	...
	return mugshot_voice(self, id, active)
end
