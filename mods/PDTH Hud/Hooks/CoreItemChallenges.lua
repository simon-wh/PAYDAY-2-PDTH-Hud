...
function MenuItemChallenge:setup_gui( node, row_item )
	if managers.menu_component._ingame_contract_gui then
		managers.menu_component._ingame_contract_gui:set_layer(0)
	end
	...
	row_item.challenge_name:set_visible( false )
	...
	row_item.description_text = row_item.gui_info_panel:text( { text = self:parameter( "description" ), layer = 1, font = tweak_data.menu.small_font, font_size = tweak_data.menu.small_font_size, color = Color.white, align = "left", vertical = "top", wrap = true, word_wrap = true } )
	...
	row_item.challenge_hl = row_item.gui_info_panel:text( { text = row_item.text, layer = node.layers.items, font = tweak_data.menu.small_font, font_size = tweak_data.menu.challenges_font_size, color = Color.white, align = "left", vertical = "left", wrap = true, word_wrap = true } )
	...
	local text = managers.localization:text( "menu_reward") .. " " .. challenge_data.xp .. " POINTS"
	row_item.reward_text = row_item.reward_panel:text( { text = text, layer = node.layers.items, font = tweak_data.menu.small_font, font_size = tweak_data.menu.challenges_font_size, color = Color.white, align = "left", vertical = "left" } )
	
	--row_item.color = chl_color
	
	local _,_,w,h = row_item.challenge_name:text_rect()
	row_item.gui_panel:set_h( h )
	row_item.gui_panel:set_layer( 5 )
	-- local bar_w = 192
	local bar_w =  node._left_align( node ) - safe_rect.width/2
	--if challenge_data.count and challenge_data.count > 1 thensf
	local bg_box = row_item.gui_info_panel:rect({	
		...
		--halign="center", 
		--vertical="center",
		color = Color.black:with_alpha(0.25), -- node.row_item_color,
		layer = 0,
		visible = true,
	})
	--bg_box:set_lefttop(row_item.challenge_name:lefttop())
	...
	local progress_text = row_item.gui_panel:text( {
		...
		layer = node.layers.items + 10,
		text = "", -- ..math.floor( params.data*100 ).."%",
		render_template = Idstring("VertexColorTextured")
	})
	--SaveTable(row_item.challenge_name.__index, "text_index.txt")
	if challenge_data.count then
		progress_text:set_text(progress_data.amount.."/"..challenge_data.count .. "   " .. row_item.challenge_name:text())
	else
		progress_text:set_text(row_item.challenge_name:text())
	end
	--row_item.challenge_name:set_text(row_item.challenge_name:text() .. "   " .. progress_data.amount.."/"..challenge_data.count)
...
function MenuItemChallenge:highlight_row_item( node, row_item, mouse_over )
	...
	row_item.challenge_name:set_visible( false )
	--row_item.challenge_name:set_font( tweak_data.menu.small_font )
	--if row_item.bar then
		row_item.progress_text:set_color( row_item.color )
		--row_item.progress_text:set_font( tweak_data.menu.small_font )
...
function MenuItemChallenge:_layout( node, row_item )
	...
	--row_item.bg_box:set_left( node._right_align( node ) - row_item.gui_panel:x() )
	...
	row_item.reward_text:set_visible(false)
...
--log("CoreItemChallenges hook done")