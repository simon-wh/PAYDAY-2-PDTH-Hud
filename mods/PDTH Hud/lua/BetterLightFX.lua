...
function BetterLightFX:Processor()
    ...
    Hooks:Add("MenuUpdate", "MenuUpdate_BetterLightFX", function( t, dt )
        ...
        --log("Status of self.routine " .. coroutine.status(BetterLightFX.routine))
    end)
        ...
        while true do
            if self._current_event then
                if self.events[self._current_event] then
                    --log("Attempting to run " .. self._current_event)
...
function BetterLightFX:RegisterEvent(name, parameters)
    if self.events[name] then
        log("[BetterLightFX] Cannot replace existing event, " .. name)
        return
    end
    ...
    log("[BetterLightFX] Registered event " .. name)
end

function BetterLightFX:DoesEventExist(name)
    if not self.events[name] then
        log("[BetterLightFX] Event does not exist, " .. name)
        return false
    else
        return true
    end
...
function BetterLightFX:StartEvent(name)
    log("[BetterLightFX] Starting event, " .. name)
    ...
    log("[BetterLightFX] Event started, " .. name)
end

function BetterLightFX:EndEvent(name)
    log("[BetterLightFX] Ending event, " .. name)
    ...
    log("[BetterLightFX] Event ended, " .. name)
end

function BetterLightFX:UpdateEvent(name, parameters)
    log("[BetterLightFX] Updating event, " .. name)
    if not self:DoesEventExist(name) then
        return
    end
    ...
    for parameter, value in pairs(parameters) do
        ...
        log("[BetterLightFX] Updating " .. name..", parameter=" .. parameter)
    end
    
     log("[BetterLightFX] Event updated, " .. name)
end
